# Set the directory where bitmaps are located
...
set spec(twos) "22222222222222222222222222222222222222222222222222"
...
# FIXED: Use 'args' to make this function robust against argument count errors
proc DrawCell { args } {
    global size color bitmapDir

    # Manually unpack arguments to ensure safety
    if {[llength $args] != 5} {
        puts "Warning: DrawCell received wrong arg count: $args"
        return
    }
    lassign $args x y s d a

    if { $s > 1 } {
    	.arena.x${x}y$y configure -bitmap  @$bitmapDir/blob$size.pbm -fg black
    } else {
    	.arena.x${x}y$y configure -bitmap  @$bitmapDir/pac${size}_$s$d$a.pbm -fg $color($s)
    }
}

proc ClearArena {} {
    global size spec1Cnt spec2Cnt display_clockCount bitmapDir
...
proc InitArena {} {
    global size bitmapDir

    for {set y 1} {$y < 10} {incr y} {
        frame .arena.r$y
	for {set x 1} {$x < 20} {incr x} {
	    label .arena.x${x}y$y -bitmap  @$bitmapDir/blob$size.pbm -bd 0
	    pack .arena.x${x}y$y -in .arena.r$y -side left
	}
    	pack .arena.r$y -side top
    }
    pack .arena -side top -anchor center
}
...
label .status.spec1pic -bitmap @$bitmapDir/pac16_000.pbm -fg $color(spec1)
...
label .status.spec2pic -bitmap  @$bitmapDir/pac16_100.pbm -fg $color(spec2)
...
# Commented out to avoid crash if missing
